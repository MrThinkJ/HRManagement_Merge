    ...
    .then(function (response) {
      ...
      new DataTable(".table-1",{
        ...
        columns: [
            ...
            { 'data': null, title: 'Action', wrap: true, "render": function (item, row) {
              const employeeId = `${row.firstName}-${row.lastName}`
              console.log(employeeId)
              return `<div class='btn-group'> <button type='button' onclick=location.href='./Edit.html' class='btn btn-success'>Edit</button><button type='button' onclick=deleteEmployee(${row.firstName}, ${row.lastName}) class='btn btn-danger btn-delete'>Delete</button></div>` }},
          ],
        searching: true,
        "bDestroy": true,
      });
